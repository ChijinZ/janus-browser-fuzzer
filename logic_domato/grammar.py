...
class CompCheckerOnGrammar:
    def __init__(self):
        self.comp_checker = CompatibilityChecker()
        self.current_css_prop = None

    def set_current_css_prop(self, prop_name):
        self.current_css_prop = prop_name

    def check_css_prop(self, prop_name) -> CompatibilityResult:
        return self.comp_checker.check_css_for_browsers(prop_name, [], ["chrome", "firefox", "webkitgtk"])

    def check_css_prop_and_value(self, prop_name, value) -> CompatibilityResult:
        return self.comp_checker.check_css_for_browsers(prop_name, [value], ["chrome", "firefox", "webkitgtk"])
...
class Grammar(object):
    ...
    def __init__(self, extract_property=False):

        self.comp_checker = CompCheckerOnGrammar()
    ...
    def _select_creator(self, symbol, recursion_depth, force_nonrecursive):
        ...
        for _ in range(1000):
            ...
            if symbol == "cssproperty":
                try:
                    css_prop_name = creator["parts"][0]["text"][:-2]
                    if self.comp_checker.check_css_prop(css_prop_name) != CompatibilityResult.Compatible:
                        continue
                    else:
                        self.comp_checker.set_current_css_prop(css_prop_name)
                        break
                except BaseException:
                    break
            elif symbol.startswith("cssproperty_") and symbol != "cssproperty_value":
                if self.comp_checker.current_css_prop is None:
                    break
                css_prop_name = symbol[12:]
                assert css_prop_name == self.comp_checker.current_css_prop, \
                    f"{css_prop_name}, {self.comp_checker.current_css_prop}, {symbol}"
                if creator["parts"][0]["type"] != "text":
                    # <cssproperty_box-flex> = <fuzzint>
                    break
                css_val_name = creator["parts"][0]["text"]
                if self.comp_checker.check_css_prop_and_value(css_prop_name,
                                                              css_val_name) != CompatibilityResult.Compatible:
                    continue
                else:
                    break
            else:
    ...
    def _parse_grammar_line(self, line):
        ...
        if self.extract_property:
            css_prop_pattern = re.compile("cssproperty_(.*)")

            match = css_prop_pattern.match(rule["creates"]["tagname"])
            if match:
                prop = match.group(1)
                if prop != "name" and prop != "value":
                    if prop not in self.property:
                        self.property[prop] = []
                    self.property[prop].append(rule)
    ...
    def parse_from_string(self, grammar_str):
        ...
        if self.extract_property:
            print(self.property)
            import json
            with open("property_dic.json", "w+") as f:
                json.dump(self.property, f)
