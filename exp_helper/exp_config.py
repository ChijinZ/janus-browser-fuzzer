...
tool_list = ["minerva", "minerva-", "domato", "freedom", "favocado"]
...
experiemtn_envs = {
    "SANCOV": {
        "Domato": {
            ...
            "REGISTRATION_ADDR": "127.0.0.1:1236",
...
def get_option() -> dict:
    ...
    parser.add_option("-f", "--fuzzer", dest="fuzzer",
                      help="choose a fuzzer (Minerva, Minerva-, Domato, FreeDom, Favocado)")
...
def get_cmd_arguments(exp_type, tool, browser, time, target_output_dir) -> (list, list):
    server_args = []
    fuzzer_args = []
...
def get_minerva_env(browser_name):
    ...
    if browser_name == "chromium":
        ...
        browser_env = "FIREFOX"
    return_envs = {"BROWSER_GRAMMAR": browser_env}
    if "MEM_DEP_JSON_PATH" not in os.environ:
        print("MEM_DEP_JSON_PATH not in the env var")
        exit()
    return return_envs


def get_minerva_ablation_env(browser_name):
    browser_env = "WEBKIT"
    if browser_name == "chromium":
        browser_env = "CHROME"
    elif browser_name == "firefox":
