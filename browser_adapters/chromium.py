...
class ChromiumSeleniumBrowser(FuzzedBrowser):
    ...
    def __del__(self):
        try:
            self.xvfb.kill()
            xvfb_file = f"/tmp/.X{self.display_port}-lock"
            if os.path.exists(xvfb_file):
                os.remove(xvfb_file)
        except:
            pass
        try:
    ...
    def launch_browser(self):
        ...
        ops.add_argument("–start-fullscreen")
        ops.add_argument("--headless")
        ops.add_argument(f"--user-data-dir={self.tmp_dir}/chrome_dev")
        try:
            # it's very very dangerous, but I can't find a better way
            # because selenium didn't export an API for setting env var
            # perhaps the lock is a good solution? I don't know
            temp_lock = Lock()
            with temp_lock:
            ...
            logging.error(f"[{self.thread_id}]: keyboard interrupt")
            raise e
        except BaseException as e:
            logging.error(f"[{self.thread_id}]: cannot launch chromium browser. {repr(e)}")
            # raise
    ...
    # Note: return true if the page is present normally
